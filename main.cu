...
__device__ bool swc2v(double * nextpos, double4 child, double4 parent, double dist)
{
  double x0 = nextpos[0];
  double y0 = nextpos[1];
  double z0 = nextpos[2];
  ...
  if (list1)
  {
    ...
    /**
     * @brief 
     * r = r1 + sqrt((x-x1).^2 + (y-y1).^2 + (z-z1).^2) / sqrt((x2-x1)^2+(y2-y1)^2+(z2-z1)^2) * (r2-r1);
  ...
  {
      pos1 = ( ( ((x0 - x1) * (x0 - x1)) + ((y0 - y1)*(y0 - y1)) + ((z0 - z1)*(z0 - z1))) < (r1*r1) ) || (( ((x0 - x2) * (x0 - x2)) + ((y0 - y2)*(y0 - y2)) + ((z0 - z2)*(z0 - z2))) < ( (r2*r2) ));
      pos = pos1;
  }
...
__global__ void
simulate(double *A,double * dx2, double *Bounds, int *LUT, int* IndexArray,double * SWC, bool *conditionalExample, curandStatePhilox4_32_10_t *state, int size, const int pairmax, int iter,
        bool debug) {
    ...
    if (gid < size) {
        ...
        for (int i = 0; i<iter; i++)
        {
          ...
          dx=(A[3*gid+0]-xnot[0])*res;
          dy=(A[3*gid+1]-xnot[1])*res;
          dz=(A[3*gid+2]-xnot[2])*res;
...
int main() {
    ...
    */
    ...
    simulate<<<grid, block>>>(d_a,devicedx2, deviceBounds, deviceLookup, deviceIndexArray,deviceSWC, deviceLogicalVector, deviceState, size, pairmax, iter, debug);
    // Wait for results
    ...
    {
      printf("xx: %.4f \t xy: %.4f \t xz: %.4f\t yy: %.4f\t yz: %.4f\t zz: %.4f\t \n", hostdx2[i*6+0], hostdx2[i*6+1], hostdx2[i*6+2],hostdx2[i*6+3],hostdx2[i*6+4],hostdx2[i*6+5]);
    }
